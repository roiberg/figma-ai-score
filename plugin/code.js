...
**Prose must match the report — never name a token your report doesn't include.** The UI renders a "Bind"/"Use" action button ONLY from an offender's \`suggestedTokens\`. If you tell the user in your summary "bind this to size/300" but size/300 isn't in that offender's \`suggestedTokens\`, you've promised a button that doesn't exist — exactly the inconsistency to avoid. For the dimensional rules (spacing, padding, size, radius) the lint's \`suggestedTokens\` are authoritative and there are NO candidate lists to choose from: an empty \`suggestedTokens\` means the lint found no unambiguous same-value token of the right category (multiple matches, or none). In that case do NOT invent one and do NOT name a specific token in your prose — say the value is hardcoded and leave token selection to the user. (Colors are the only exception: they carry \`_allTokenCandidates\` precisely so you CAN pick — and whatever you pick must end up in \`suggestedTokens\`.)
...
async function handleRpc(method, params) {
  switch (method) {
    ...
    case "begin_and_scan": {
      ...
      const _dsTimeout = new Promise(r => setTimeout(() => r(null), 12000));
      ...
      if (!designSystem) {
        try { designSystem = await Promise.race([getDesignSystem(), new Promise(r => setTimeout(() => r(null), 12000))]); } catch (e) {}
      }
